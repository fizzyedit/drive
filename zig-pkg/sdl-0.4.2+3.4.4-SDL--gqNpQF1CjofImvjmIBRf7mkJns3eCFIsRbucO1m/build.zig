...
pub fn build(b: *std.Build) void {
    ...
    if (ios) {
        // NOTE: no -pthread — iOS clang rejects it (threads are always available).
        sdl_c_flags.appendAssumeCapacity("-fobjc-arc");
    }
    if (emscripten and emscripten_pthreads) {
        sdl_c_flags.appendAssumeCapacity("-pthread");
    ...
    if (ios) {
        // NOTE: UIKit/Metal instead of Cocoa; MFI-only joystick; Vulkan omitted (no MoltenVK dep).
        sdl_mod.addCSourceFiles(.{
            .flags = sdl_c_flags.items,
            .files = &.{
                "src/audio/dummy/SDL_dummyaudio.c",
    ...
    if (ios) {
        ...
        // NOTE: SDL_uikitopengles.m/SDL_uikitopenglview.m use EAGL (SDL_VIDEO_OPENGL_ES=true).
        sdl_mod.linkFramework("OpenGLES", .{});
        // NOTE: SDL_mfijoystick.m uses CHHapticEngine for controller rumble.
        sdl_mod.linkFramework("CoreHaptics", .{ .weak = true });
    }
